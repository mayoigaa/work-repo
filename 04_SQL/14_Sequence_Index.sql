-- 14_ Sequence_Index.sql
-- 1. 시퀀스(Seqence) : 호출(실행)할 때마다 자동적으로 숫자를 증가시키는 것( 객체 )
-- 목적) 기본키 용도로 사용(중복 없음 ,null 없음) 
-- 활용 예) 자유 게시판 : No(1 ~ n), 고객: 고객No(1 ~ n)
-- 실무 용어 : 채번 : 자동적으로 1씩 증가시키는 값(1 ~ n: 시퀀스대상)
...
-- 현재 증가된 시퀀스의 값을 확인 : 최초 1번은 시퀀스명.NEXTVAL로 실행 후 확인 가능
-- 사용법) 시퀀스명.CURRVAL
SELECT SAMPLE_SEQ.CURRVAL FROM DUAL;

-- 시스템 테이블들 : 시퀀스 확인용 테이블(뷰) : 데이터 사전 (DICTIONARY VIEW)
...
-- 목적) 조회속도 향상 위해 조건에 해당한느 컬럼에 인덱스를 생성함(지정함) 
-- 기본키(PK) : 중복 방지 + NULL값방지 + 자동인덱스생성 

-- 예제 3) 사이트에서 조회시 이름검색이 (ENAME) 많은데 속도가 느리다고 한다. 
-- 해결) ENAME 컬럼에 인덱스 생성 -> 조회속도  향상 
-- 명명법) IX_테이블명_컬럼명, IDX_테이블명_컬럼명
-- 사용법) CREATE INDEX 인덱스 이름 ON 테이블명(컬럼명);
CREATE INDEX IX_EMPLOYEE_ENAME
ON EMPLOYEE(ENAME);

-- 속도 느림 대상 SQL문
SELECT * FROM EMPLOYEE
WHERE ENAME LIKE 'SCOTT%'; -- 이 부분이 INDEX를 만들 곳 
