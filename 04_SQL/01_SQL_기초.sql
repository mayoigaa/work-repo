-- sql developer 툴 : DB 점속툴(== DB 클라이언트 툴)
-- oracle 설치      : DB 서버 
-- DB 서버 <- 사용자가 접속해서 사용함 (계정/암호)
-- system 계정 : 관리자 (계정 생성, 권한 관리 또는 DB 환경관리 용으로 사용) 
...
-- 1)SQL문 : 검색(조회) : SELECT ~ FROM 문
-- 사용법) SELECT 컬럼명, 컬럼명2, ... FROM 테이블명;
--          * : 모든 컬럼 
--SELECT * FROM DEPARTMENT;  -- 전체 부서를 조회하는 쿼리 (질의문)
...
-- 예제 3) 부서테이블의 컬럼 중에서 DNO, LOC 컬럼만 조회하기 
...
-- 사원 테이블에 부서번호(DNO)를 중복을 제거해서 화면에 출력하세요 
