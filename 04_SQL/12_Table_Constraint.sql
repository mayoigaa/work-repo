...
-- 기본키의 특징 : 유일한 값만 입력되고(중복방지) + NOT NULL 제약 조건포함 + (참고:INDEX 자동생성)
-- 특징) 주로 테이블설계씨 1개의 테이블당 거의 1개는 기본키가 존재함
-- 기본키 제약조건에 이름을 부여 : CONSTRAINT 제약조건이름 제약조건 
-- 제약조건이름 붙이기 약속 (명명범) : PK_테이블명_컬럼명, UK_테이블명_컬럼명 ...
-- 사용법) CREATE TABLE 테이블명 (
--               컬럼명 자료형(크기)
...
VALUES(NULL, NULL, NULL, '010-1234-4567', 'SEOUL'); -- 기본키로 인해 에러발생

-- 3) 외래키(참조키) 제약조건: FROGEIN KEY(FK)
...
--        ADD CONSTRAINT PK_테이블명_컬럼명 PRIMARY KEY(ENO);
-- ENO : 사원번호 (기본키) 
ALTER TABLE EMP_COPY
ADD CONSTRAINT PK_EMP_ENO PRIMARY KEY(ENO);
