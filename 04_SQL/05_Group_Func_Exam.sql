...
-- 컬럼의 별칭은 : 최고액, 최저액, 총액, 평균 급여
-- 단 실수가 나오면 반올림해주세요(ROUND)
-- 사원 : EMPLOYEE
...
-- 단, 관리자를(MANAGER) 알 수 없는 사원 및 최저 급여가 2000 미만인 그룹을 제외시키고,
...
-- 부서별로(DNO) 몇명이 입사했는지 출력하세요 
-- 힌트) 입사년도 : TO_CHAR(HIREDATE, 'YYYY')
SELECT DNO,TO_CHAR(HIREDATE, 'YYYY'), COUNT(*) AS CNT
FROM EMPLOYEE
