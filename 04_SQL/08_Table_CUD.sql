...
-- 1) 테이블 주석 달기 
-- 사용법) COMMENT ON TABLE DEPT IS '테이블 주석';
...
COMMENT ON COLUMN EMP.ENO IS '사원명';
COMMENT ON COLUMN EMP.ENAME IS '직위';
...
COMMENT ON COLUMN EMP.COMMISSION IS '사원번호';
...
-- 의미 : 영구 반영하면 다른 유저가 현재 생성한 수정/삭제한 데이터를 볼 수 있다. 

COMMIT;


-- 예제 7) 임시적으로 NULL 값을 입력해서 INSERT 하기
-- 1) 암묵적으로(눈에 안보이지만) NULL값 INSERT하기
...
-- 예제 9) INSERT할 때 현재날짜(시간) 데이터 넣기 : SYSDATE (현재날짜)
INSERT INTO EMP_COPY(ENO, ENAME, JOB, HIREDATE, DNO)
VALUES(7010, 'CANDY3','MANAGER', SYSDATE, 10);
-- 영구 반영
COMMIT;
SELECT * FROM EMP_COPY;
