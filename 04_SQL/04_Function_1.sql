...
-- 연습 1) 사원테이블(EMPLOYEE)에서 사원명(ENAME)을 소문자로 출력하고,
...
-- 연습 2) SCOTT 사원을 조회해서 사원 번호 (ENO), 사원명(ENAME), 부서번호(DNO) 출력하기
-- 단, SCOTT이 대소문자 구분없이 비교된다고 가정하고 작성하세요 
SELECT ENO, ENAME, DNO FROM EMPLOYEE
WHERE ENAME = UPPER('SCOTT'); -- 어쨌든 SCOTT 사원의 정보를 출력

-- 1-2) 문자 길이를 RETURN 하는 함수
-- LENGTH(문자열) : 한글/영어 상관없이 문자개수를 세어서 RETURN하는 함수
-- 코딩 언어셋 : 영어표현 (1BYTE), 한글 표현(3BYTE) [UTF-8 표현식(국제표준)]
...
-- SQL : 인덱스번호(1부터 시작)
-- 사용법) SUBSTR (대상 컬럼, 시작위치, 자를 개수)
-- 단, 시작위치가 음수이면 뒤에서 셈
SELECT SUBSTR('Oracle mania',4,3)
      ,SUBSTR('Oracle mania',-1,1)
FROM DUAL;

-- 예제 3) 이름이 N으로 끝나느 사원 화면에 출력하기
-- 단, SUBSTR() 함수를 이용하세요
-- 사원 : EMPLOYEE
...
-- 사용법) RPAD(대상컬럼, 자리수, 채울 문자)
--            (자리수에서 빈칸은 채울 문자로 채워짐(오른쪽))
SELECT RPAD(SALARY, 10, '*')
FROM EMPLOYEE;

-- 1-7) 문자열에서 공백제거하기 : LTRIM, RTRIM, TRIM
-- 사용법) LTRIM(컬럼명) : 왼쪽 공백 제거 
-- 사용법) RTRIM(컬럼명) : 오른쪽 공백 제거 
-- 사용법) TRIM(컬럼명) : 양쪽 공백 제거 
