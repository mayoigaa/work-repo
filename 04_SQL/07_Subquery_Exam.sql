...
-- 2) 1600 보다 큰 사원의 사원이름(ENAME),담당업무(JOB)를 화면에 표시
SELECT ENAME, JOB FROM EMPLOYEE
...
-- 7) 매니저가 (MANAGER) 있는 사원의 이름을 표시하세요
-- 단, 서브쿼리를 이용하세요 
-- 사원테이블 : EMPLOYEE
...
-- 2073.214285714285714285714285714285714286 보다 많은 급여를 받는 사원들 출력 
SELECT ENO,ENAME FROM EMPLOYEE
WHERE SALARY > (SELECT AVG(SALARY) FROM EMPLOYEE)
ORDER BY SALARY;

-- 10) (이름에 (ENAME)K가 포함된 사원)과 같은 부서에서 (DNO) 일하는 사원의
-- 사원 번호와 (ENO) 이름을(ENAME) 표시하는
-- 질의를 작성하세요
-- 사원테이블 :EMPLOYEE
...
-- 11) (부서위치가 (LOC) DALLAS인) 사원의 이름과 (ENAME)
...
-- 힌트 : 보고하는 사원의 매니저가 (MANAGER) KING임
-- 사원테이블 : EMPLOYEE

-- 1) KING 의 사번(ENO) 구하기 : 7839
SELECT ENO FROM EMPLOYEE
WHERE ENAME = 'KING';

-- 2) 7839가 MANAGER인 사원의 사원명과 (ENAME) 급여를 (SALARY) 조회하면 됨
SELECT ENAME, SALARY FROM EMPLOYEE
WHERE MANAGER = (SELECT ENO FROM EMPLOYEE
                 WHERE ENAME = 'KING');

-- 13) (RESEARCH 부서(DNO)의 사원에 대한 부서번호(DNO),
--      사원 이름 (ENAME) 및 담당 업무(JOB)를 표시하세요.
SELECT DNO FROM DEPARTMENT
