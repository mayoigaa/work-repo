...
-- 예제 1) 최근에 입사한 (HIREDATE) 사원과 (EMPLOYEE) 가장 오래전에 입사한 사원의 입사일 출력하기 
-- 힌트) 위의 집계함수들 중 2개를 사용해야한다
-- 최근 입사일 : MAX (입사일)
-- 오래전 입사일 : MIN (입사일)
...
-- COMMISSION(상여금) 컬럼에는 NULL이 있음 : 하지만 집계함수는 NULL 자동으로 제외하고 계산함 
SELECT SUM(COMMISSION) FROM EMPLOYEE;

-- 예제3) 사원들의(EMPLOYEE) 총인원 출력하세요
-- 사용법) COUNT(컬럼명[*])
SELECT COUNT(*) AS 사원수
FROM EMPLOYEE;
...
-- 사원 : EMPLOYEE
-- DISTINCT : 중복 제거 키워드 
-- 사용법) 중복 제거 COUNT 함수: COUNT(DISTINCT 컬럼명)
-- 컬럼 별칭 : 컬럼명 AS 별칭
...
FROM EMPLOYEE; -- 에러 (일반 컬럼(ENAME), MAX(SALARY) 같이 사용 못함 
...
-- 예제 9) 부서병(DNO) 최고급여가 (SALARY) 3000 이상인 부서의 번호(DNO)와
--         최고급여금액 구하기
...
-- 연습 1) 사원테이블에서 (EMPLOYEE) JOB(직위)이 1)MANAGER인
