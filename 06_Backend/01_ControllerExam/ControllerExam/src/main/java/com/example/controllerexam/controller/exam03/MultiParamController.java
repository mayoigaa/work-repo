...
//    URL 테스트 :  http://localhost:8000/exam03/hello-name-id?name=kang&id=sushidaisuki
...
//    url 테스트 : url 테스트 :
   @GetMapping("/example01")
    public String setHelloDnoDname(Model model,
                                   @RequestParam(defaultValue = "0")int dno,
                                   @RequestParam(defaultValue = "")String dname,
                                   @RequestParam(defaultValue = "")String loc
   ){
        model.addAttribute("attrDno","부서번호: "+ dno);
        model.addAttribute("attrDname","부서명: "+ dname);
        model.addAttribute("attrLoc","위치: "+ loc);
